  ...
  chrome.runtime
    ...
    .catch((error) => {
      if (error.message.includes("Could not establish connection")) {
        ...
        console.error("Background: Error sending login status message:", error);
      }
    });
}
...
async function handleAuthCallback(tabId, url) {
  console.log("Background: Handling auth callback for tab", tabId, "URL:", url);

  try {
    // First check if the tab's title contains the signal
    chrome.tabs.get(tabId, async (tab) => {
      if (tab.title === "AUTH_SUCCESS") {
        console.log("Background: Auth success detected by title");

        // Now fetch the actual session data
        try {
          console.log("Background: Fetching session from localhost...");
          ...
          if (response.ok) {
            ...
            if (session && Object.keys(session).length > 0 && session.user) {
              ...
              // Notify any open popup that login status changed
          ...
          await chrome.storage.local.remove(["isLoggedIn", "user"]);
        } catch (error) {
          console.error("Background: Error fetching session:", error);
          await chrome.storage.local.remove(["isLoggedIn", "user"]);
        }
      }
    });
